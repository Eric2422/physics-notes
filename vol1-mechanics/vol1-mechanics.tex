...
\begin{itemize}
    \item
        \(x_0\) is the initial position, \(x_\mathrm{f}\) is the final position,
        and \(\Delta{x}\) is the change in position;
    \item \(v_0\) is the initial velocity, and \(v_\mathrm{f}\) is the final velocity;
...
\begin{itemize}
    ...
    \item
        Plumb lines
        (in AP Physics C:~Mechanics, I did this with irregularly-shaped cardboard cutouts)
        \begin{enumerate}
...
Similarly to the linear kinematic equations under constant linear acceleration (\cpageref{eq:linear-kinematic}),
there are \textbf{rotational kinematic equations} under constant \textbf{angular acceleration},
namely
\begin{subequations}
    \begin{gather}
        {
...
Note that the \(r\) in these formulas means that objects have lower moment of inertia
...
\(I\) is the moment of inertia of the object spinning around its axis of rotation,
...
Let \(r\) be the distance between the axis of rotation and the rod's center of mass
