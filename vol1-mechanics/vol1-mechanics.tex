...
where
\begin{itemize}
    \item \(x_0\) is the initial position, \(x_\mathrm{f}\) is the
        final position,
        and \(\Delta{x}\) is the change in position;
    \item \(v_0\) is the initial velocity, and \(v_\mathrm{f}\) is
        the final velocity;
...
\begin{equation}
    r_{\mathrm{c.m.}} = \frac{\int r \lambda(r) \odif{r}}{\int
    \lambda(r) \odif{r}}.
\end{equation}
With this equation,
...
\begin{equation}
    \begin{aligned}
        ...
        r^2 \rho \odif{r} \\
        &= \frac{4 G \pi \rho}{R_\mathrm{i}^2} \int_{0}^{R_\mathrm{i}}
        r^2 \odif{r} \\
