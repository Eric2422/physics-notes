...
This idea comes back up again in \nameRef{sec:capacitors}.
...
which is called "\textbf{voltage}" (\(V\))
...
\textbf{Capcitors} are a way to store charge.
Unlike batteries, they are purely electrical rather than electrochemical.
Physically, they are two electrically conductive plates separated by a distance.
...
where
\begin{itemize}
    \item \(q\) is the charge on each plate.
    \item \(V\) is the voltage between the two plates.
\end{itemize}

The voltage across the capacitor is linear,
which means that the electric field across a capacitor is uniform.

Capacitance is also given by the formula
\begin{equation}
...
the \textbf{\(\bm{B}\) field} and the \textbf{\(\bm{H}\) field}.
For this volume, we are only concerned with the \(\bm{B}\) field.
