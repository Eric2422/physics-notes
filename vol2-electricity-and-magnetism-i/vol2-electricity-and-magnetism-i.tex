...
\begin{equation} \label{eq:ohms-law}
    V = I R
\end{equation}
where \(R\) is the \textbf{resistance}, which has SI units of ohms
(\unit{\ohm}).
...
\begin{enumerate}
    \item
        ...
        \end{equation}
        This follows from the conservation of charge (\cpageref{sec:
        electric-charges}).
    \item
        \textbf{Kirchoff's loop rule} states that all the voltages in
        a closed circuit
...
\begin{equation}
    \begin{aligned}
        I V
        &= \ab( \unit{\joule \per \coulomb} ) \ab( \unit{\coulomb
        \per \second} )
