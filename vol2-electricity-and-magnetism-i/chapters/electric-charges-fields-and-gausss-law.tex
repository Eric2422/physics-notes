...
with arrrows pointing in the direction of the field
...
\begin{equation}
    \Phi = \iint_S \vecb{F}(\vecb{r}) \cdot \odif{\vecb{A}},
\end{equation}
where \(\vecb{A}\) is the area vector
...
\begin{itemize}
    ...
    \item \(\vecb{E}\) is the electric field vector,
    \item \(\vecb{A}\) is the area vector.
...
\begin{itemize}
    ...
    \item \(\Phi_E\) be the electric flux,
...
the electric field and surface area are orthogonal at all points:
\begin{equation}
    \vecb{E} \cdot \odif{\vecb{A}} = E \odif{A}.
\end{equation}
Since electric field is of equal magnitude at all points,
we can multiply the electric field by area to find the flux is
\begin{equation}
    \Phi_E = 4 E \pi r^2.
\end{equation}
...
We can also apply Gauss's law to the electric field inside of a charged sphere.
If we draw our imaginary surface inside a sphere,
the charge outside of it can be ignored.
Let
\begin{itemize}
    \item \(r\) be the distance from the center of the sphere,
    \item \(R_\mathrm{S}\) is the radius of the imaginary Gaussian surface,
    \item \(R\) be the radius of the sphere,
    \item \(\rho(r)\) be the charge density as a function of radius.
\end{itemize}
The charge inside the sphere is thus
\begin{equation}
    q = \int_0^{\mathrm{R_\mathrm{S}}} 4 \rho(r) \pi r^2 \odif{r}.
\end{equation}
Similarly to the above derivation of Coulomb's law,
the flux \(\Phi_E\) is \(4 E \pi r^2\),
so we find
\begin{equation}
    \begin{aligned}
        4 E \pi R_\mathrm{S}^2
        &= \frac{
            \int_0^{\mathrm{R_\mathrm{S}}} 4 \rho(r) \pi r^2 \odif{r}
        }{\epsilon_0} \\
        E
        &= \frac{
            \int_0^{\mathrm{R_\mathrm{S}}} \rho(r) r^2 \odif{r}
        }{R_\mathrm{S}^2 \epsilon_0} \\
    \end{aligned}
\end{equation}
In the case where \(\rho(r)\) is a constant density \(\rho\),
we find
\begin{equation}
    \begin{aligned}
        E
        &= \frac{
            \int_0^{\mathrm{R_\mathrm{S}}} \rho r^2 \odif{r}
        }{R_\mathrm{S}^2 \epsilon_0} \\
        &= \frac{
            \rho R_\mathrm{S}^3
        }{3 R_\mathrm{S}^2 \epsilon_0} \\
        &= \frac{
            \rho R_\mathrm{S}
        }{3 \epsilon_0} \\
    \end{aligned}
